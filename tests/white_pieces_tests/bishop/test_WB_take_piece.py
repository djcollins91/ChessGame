...
class TestBoard(unittest.TestCase):
    ...
    def test_take_WB(self):
        ...
        self.assertIsInstance(self.board.grid[to_y][to_x], White_Bishop, "Target position should have the Black_Bishop after the move.")
        self.assertEqual(str(self.board.grid[to_y][to_x]), White_Bishop.get_piece_str(), "Target position should have the Black_Bishop.")
        ...
        # Test 5: Invalid capture (blocked by another piece)
        self.board.place_piece(White_Bishop(White_Bishop.get_piece_str()), 4, 4)
        self.board.place_piece(White_Bishop(White_Bishop.get_piece_str()), 5, 3)
        self.board.place_piece(Black_Bishop(Black_Bishop.get_piece_str()), 6, 2)
        from_x, from_y = 4, 4
        to_x, to_y = 6, 2
        result = self.board.grid[from_y][from_x].move(self.board, from_x, from_y, to_x, to_y)
        self.assertFalse(result, "Move should be identified as invalid.")
