...
class TestBoard(unittest.TestCase):
    ...
    def test_valid_move_forward(self):
        ...
        self.board.place_piece(Black_King('BK'), 1, 4)
        from_x, from_y = 1, 4
        to_x, to_y = 2, 4
        ...
        self.board.place_piece(Black_King('BK'), 0, 1)
        from_x, from_y = 0, 1
        to_x, to_y = 0, 2
        result = self.board.grid[from_y][from_x].move(self.board, from_x, from_y, to_x, to_y)
        self.assertEqual(result, "Valid move", "Move should be identified as valid.")
        ...
        self.board.place_piece(Black_King('BK'), 6, 6)
        from_x, from_y = 6, 6
        to_x, to_y = 6, 5
        result = self.board.grid[from_y][from_x].move(self.board, from_x, from_y, to_x, to_y)
        self.assertEqual(result, "Valid move", "Move should be identified as valid.")
        ...
        self.board.place_piece(Black_King('BK'), 3, 3)
        self.board.place_piece(Black_King('BK'), 3, 2)  # Place another white Piece in front
        from_x, from_y = 3, 3
        ...
        self.assertEqual(str(self.board.grid[from_y][from_x]), 'BK', "Original position should still have the white Piece.")
        self.assertEqual(str(self.board.grid[to_y][to_x]), 'BK', "Target position should still have the white Piece.")
        print("Test 6 for BK_move passed")


        # Test 7: Testing if piece can't move two spot
        self.board.place_piece(Black_King('BK'), 1, 5)
        from_x, from_y = 1, 5
