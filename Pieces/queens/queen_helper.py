...
def queen_take_piece(board,from_y,from_x,to_y,to_x, piece_str, target_str, C):
    ...
    if piece and str(piece) == piece_str:
        ...
        if (target is not None) and str(target).startswith(target_str):
            #checks to make sure there isn't a piece in the way
            while (change_y != to_y) or (change_x != to_x):
                change_y += value_change_y
                change_x += value_change_x
                #makes sure it's a our piece isn't in the way
                if ((board.grid[change_y][change_x]) is not None) and str(board.grid[change_y][change_x]).startswith(C):
                    return Piece.invalid_move()
