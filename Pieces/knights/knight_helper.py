...
def knight_move(board,from_y,from_x,to_y,to_x, piece_str):
    ...
    if (piece and str(piece) == piece_str) and (board.grid[to_y][to_y] is None):
...
def knight_take_piece(board,from_y,from_x,piece_str,to_y,to_x, target_str):
    ...
    if (piece and str(piece) == piece_str) and (target is not None) and str(target).startswith(target_str):
        # Ensure it's a valid move
